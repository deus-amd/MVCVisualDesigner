...
{
    public partial class JavaScriptGeneratorOptionUI : UserControl
    {
        public JavaScriptGeneratorOptionUI()
        {
            InitializeComponent();
        ...
        void txtPath_Leave(object sender, EventArgs e)
        {
            ...
            string curPath = SettingsHelper.getPathFromView(SettingsHelper.GENERATED_JAVASCRIPT_PATH, view, m_rootViewPath);
            string newPath = this.txtPath.Text;
            // path not changed
            if (string.Compare(curPath, newPath, true) == 0) return;

            if (string.IsNullOrEmpty(newPath))
                SettingsHelper.savePathToView(SettingsHelper.GENERATED_JAVASCRIPT_PATH, string.Empty, view, m_rootViewPath);
            else
                SettingsHelper.savePathToView(SettingsHelper.GENERATED_JAVASCRIPT_PATH, newPath, view, m_rootViewPath);
        }
        ...
        void tvAllViews_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            if (view != null)
            {
                string path = SettingsHelper.getPathFromView(SettingsHelper.GENERATED_JAVASCRIPT_PATH, view, m_rootViewPath);
